...
- **단일 시계** — 모든 시각 앵커·비교·sweeper 선택은 `redis.call('TIME')`으로 통일한다(앱 시계 스큐를 correctness가 아닌 latency 문제로 강등).
- **sweeper 단독 구동 타이머** — 타임아웃·`REVEAL` 전이의 유일 구동기는 `RoundDeadlineSweeper`(`@Scheduled` ~1초 + `@SchedulerLock`, 단일 replica). 별도 로컬 타이머가 없어 replica마다 타이머가 중복되지 않는다. sweeper가 주 구동기이므로 `lockAtMostFor`는 `PT1M`이 아니라 폴링의 2~4배(`PT4S`)로 짧게 잡는다. 정밀이 필요한 첫 정답은 sweeper가 아니라 들어온 채팅 메시지가 즉시 처리한다(이벤트 구동).
- **Redis 키** — `room:{id}:round`(Hash: roundSeq·phase·deadlineAt·trackIndex·winnerId·trackOrder·trackDurations) / `rounds:deadlines`(전역 ZSET, score=deadlineAt·member=roomId) / `room:{id}:scores`(ZSET, 멤버 조건부 가점·퇴장 제거). 모두 GC 백스톱 TTL(24h) + 명시적 teardown(게임 종료·방 삭제).
