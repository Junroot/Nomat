                ...
                <Column1>
                    ...
                    <div className="w-full p-4 flex flex-col gap-1 bg-zinc-800 rounded-2xl">
                        <div className="inline-flex items-end gap-1 text-2xl font-bold">
                            <PlaylistIcon className="size-8" />
                            <p>{playlistTitle}</p>
                        </div>
                        <p className="text-md text-zinc-400">by. {playlistMaster}</p>
                    ...
                    <div className="w-full p-4 flex flex-col gap-2 bg-zinc-800 rounded-2xl">
                        <div className="inline-flex items-end gap-1 text-2xl font-bold">
                            ...
                            <p>플레이어</p>
                            <p className="text-lg">{mockPlayers.length}/16</p>
                        </div>
                        {mockPlayers.map((player) => (
                            <div key={player.id} className="flex items-center gap-3 p-2 hover:bg-zinc-700/50 cursor-pointer rounded-lg transition-colors duration-200">
                                <div className="relative">
                                    <UsersIcon className="size-10 rounded-full border border-zinc-600" />
                                    <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-neon-green border-2 border-zinc-800" />
                                </div>
                                <div className="flex items-center gap-2">
                                    <p className="text-zinc-200">{player.nickname}</p>
                                    {player.isMaster && (
