...
description: 실험적 워크플로에서 완료된 변경을 아카이브한다. 구현이 끝난 뒤 변경을 마무리하고 아카이브하고 싶을 때 사용한다.
license: MIT
...
**입력**: 변경 이름을 선택적으로 지정한다. 생략하면 대화 맥락에서 추론할 수 있는지 확인한다. 모호하거나 불분명하면 반드시 사용 가능한 변경 목록을 제시해 물어봐야 한다.
...
2. **아티팩트 완료 상태를 확인한다**
   ...
   **`done`이 아닌 아티팩트가 있으면:**
   ...
   - **AskUserQuestion tool**을 사용해 진행할지 사용자에게 확인받는다
   - 사용자가 확인하면 진행한다
...
**스펙:** ✓ 메인 스펙에 동기화됨 (또는 "델타 스펙 없음" 또는 "동기화 건너뜀")
...
- 동기화를 요청하면 openspec-sync-specs 방식을 사용한다(에이전트 주도)
