---
name: "OPSX: Verify"
description: archive 전에 구현이 change artifact와 일치하는지 검증한다
category: Workflow
...
**입력**: `/opsx:verify` 뒤에 change 이름을 선택적으로 지정한다(예: `/opsx:verify add-auth`). 생략하면 대화 맥락에서 추론할 수 있는지 확인한다. 모호하거나 불분명하면 반드시 사용 가능한 change 목록을 제시해 선택을 요청해야 한다.
