# redis-pubsub-health Specification

## Purpose

Redis pub/sub subscription의 살아있음을 self round-trip ping으로 검증해 백엔드 인스턴스의 도메인 이벤트 수신 경로가 실제로 동작 중임을 헬스체크로 노출하는 역량을 정의한다. Spring Boot Actuator `HealthIndicator`를 통해 `/actuator/health` 및 `/health` 엔드포인트에 결과가 반영되며, Docker Swarm·nginx 등 외부 인프라가 보는 컨테이너 헬스체크와 직접 연결된다. 도메인 이벤트(`room:*:events` 등)와 동일한 dedicated pub/sub connection을 공유하여 이벤트 채널의 단절을 헬스체크로 즉시 감지할 수 있게 한다.
