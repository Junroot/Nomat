...
시스템은 `REVEAL` 단계를 **10초**간 유지한 뒤, 남은 트랙이 있으면 별도의 카운트다운 없이 다음 라운드를 `OPEN`으로 자동 진행하고, 마지막 트랙이면 게임을 `ENDED`로 종료해야(SHALL) 한다. REVEAL 길이는 **어떤 경로로 진입했든**(첫 정답·타임아웃·포기 임계 도달·퇴장으로 인한 임계 도달) 방 설정과 무관하게 항상 10초로 고정한다. 전이는 서버 타이머가 구동한다.
...
#### Scenario: 진입 경로가 달라도 REVEAL 길이는 같다
- **WHEN** 참가자의 첫 정답으로 `REVEAL`에 진입
- **THEN** 아무도 맞히지 못해 타임아웃으로 진입한 경우와 동일하게 10초간 유지되어야 한다

#### Scenario: 포기로 열린 REVEAL도 같은 길이를 갖는다
- **WHEN** 포기 인원이 임계에 도달해 `REVEAL`에 진입(직접 포기하거나, 퇴장으로 남은 인원이 줄어 임계를 넘긴 경우 모두)
- **THEN** 정답·타임아웃으로 진입한 경우와 동일하게 10초간 유지되어야 한다
