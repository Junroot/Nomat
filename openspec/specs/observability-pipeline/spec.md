...
ELK(Elasticsearch/Logstash/Kibana) 기반 로깅과 self-hosted Prometheus/Grafana를 Grafana Cloud + Grafana Alloy 단일 에이전트 파이프라인으로 대체하는 관측성(observability) 파이프라인 역량을 정의한다. 백엔드·인프라 컴포넌트의 로그·메트릭을 Alloy가 단일 에이전트로 수집·송신하며, 로그는 Grafana Cloud Loki로, 메트릭은 Mimir로 보낸다. 라벨은 low-cardinality만 사용하고 요청 단위 식별자는 로그 라인 본문으로 보존하며, 앱 로그와 access 로그는 동일 송신 경로에서 `logType` 라벨로 구분한다. 운영 모니터링 UI는 Grafana Cloud Grafana로 단일화하고, Elasticsearch는 플레이리스트 검색 책임만 유지한다. Grafana Cloud 자격증명은 환경변수로만 주입한다.
